...
bool host_video_open(const char *title, int width, int height, int scale)
{
    ...
    // and the options screen prints a single digit because a zoom of 12 is not a
    ...
    // real ones. So on a 200% display the same zoom gives a window of the same
    // apparent size under Wayland and half of it under X11. Someone who wants
    // more raises the zoom, and the number still means what it says.
    {
        const char *want = getenv("FRLG_SCALE");
        char line[96];

        // Same order as everywhere else: the environment is what a run was
        // told, the setting is what a person chose.
        if (want != NULL)
