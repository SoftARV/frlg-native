...
static void DrawRows(void)
{
    ...
    if (sMenu->count == 0)
    {
        AddTextPrinterParameterized3(0, FONT_NORMAL, 8, 24, sTextColour,
                                     TEXT_SKIP_DRAW, sNothing);
    }

    for (i = 0; i < sMenu->count; i++)
    {
        u8 y = 24 + i * ROW_HEIGHT;
        int id = sMenu->rows[i];
    ...
    AddTextPrinterParameterized3(0, FONT_NORMAL, 8, 24 + MAX_ROWS * ROW_HEIGHT,
